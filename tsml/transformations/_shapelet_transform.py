    ...
    for i_dilation in prange(n_dilations):
        ...
        for i_shp in id_shps:
            ...
            if n_admissible_points > 0:
                ...
                if norm:
                    x_dist = _compute_normalized_shapelet_dist_vector(
        ...
        for i_x in prange(n_instances):
            ...
            if len(idx_norm) > 0:
                ...
                for i_shp in idx_norm:
                    X_new[
                        i_x, (n_ft * i_shp) : (n_ft * i_shp + n_ft)
                    ] = _compute_shapelet_features_normalized(
                        X[i_x],
                        values[i_shp],
                        length,
                        dilation,
...
@njit(fastmath=True, cache=True)
def _compute_shapelet_features(X, values, length, dilation, threshold):
...
@njit(fastmath=True, cache=True)
def _compute_shapelet_features_normalized(
...
@njit(fastmath=True, cache=True)
def _compute_normalized_shapelet_dist_vector(X, values, length, dilation, means, stds):
